...
def to_parquet(table_name: str, lnf: nw.LazyFrame[duckdb.DuckDBPyRelation]) -> nw.LazyFrame[duckdb.DuckDBPyRelation]:
    with con.cursor() as cur:
        cur.execute(f"COPY ({to_duckdb(lnf).sql_query()}) TO '{here(f"data/local/{table_name}")}' (FORMAT parquet, COMPRESSION ZSTD, FILENAME_PATTERN='{table_name}_{{i}}', FILE_SIZE_BYTES 2_000_000_000);")
        cur.execute(f"CREATE OR REPLACE VIEW books.{table_name} AS SELECT * FROM read_parquet('{here(f'data/local/{table_name}/{table_name}_*.parquet')}')")
...
for file in here(f"data/local").glob("*/*.parquet"):
