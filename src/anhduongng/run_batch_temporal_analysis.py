...
# Cities and their spelling search patterns
city_patterns = {
    "berlin": ["%berlin%", "%colln%"],
    "nürnberg": ["%nürnberg%", "%nuremberg%", "%noriberg%", "%norimberg%"],
    "straßburg": ["%straßburg%", "%strasburg%", "%strasbourg%", "%argentorat%"],
    "halle": ["%halle%", "%halis%"],
    "helmstedt": ["%helmstedt%", "%helmstadt%", "%helmstadi%", "%helmaestadi%"],
    "hamburg": ["%hamburg%"],
    "dresden": ["%dresden%", "%dresda%"],
    "rostock": ["%rostock%", "%rostoch%"],
    "wien": ["%wien%", "%vienna%", "%vindobon%"],
    "tübingen": ["%tübingen%", "%tuebingen%", "%tubing%"],
    "erfurt": ["%erfurt%", "%erford%"],
    "basel": ["%basel%", "%basil%"],
    "göttingen": ["%göttingen%", "%goettingen%", "%gotting%"],
    "münchen": ["%münchen%", "%munich%", "%monach%"]
}
...
# 3. BATCH RUNNER
# ═══════════════════════════════════════════════════════════════════════════
print(f"Connecting to database to start batch processing...")
with dbapi.connect(db_params["uri"], db_kwargs=dict(username=db_params["username"], password=db_params["password"])) as db_con:
    for city, pats in city_patterns.items():
        ...
        # Build SQL condition
        sql_conds = [f"p.place_of_publication ILIKE '{pat}'" for pat in pats]
        where_clause = " OR ".join(sql_conds)
...
print("\nBATCH PROCESSING COMPLETED SUCCESSFULLY FOR ALL CITIES!")
